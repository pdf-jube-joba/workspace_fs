...
For `workspace_fs` plugins, pass `{OUTPUT_DIRECTORY}` and configure any optional enhancers under `[plugin.md_preview]`. Each enhancer must specify both `url` and `entrypoint`.
